...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING]:
                ...
                if download.status() in [MirrorStatus.STATUS_DOWNLOADING,
                    ...
                    msg += f"\n<b>✅️ Downloaded:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                elif download.status() == MirrorStatus.STATUS_UPLOADING:
                    msg += f"\n<b>✅️ Uploaded:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                elif download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n<b>✅️ Cloned:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                elif download.status() == MirrorStatus.STATUS_ARCHIVING:
                    msg += f"\n<b>✅️ Archived:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                elif download.status() == MirrorStatus.STATUS_EXTRACTING:
                    msg += f"\n<b>✅️ Extracted:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
        ...
        if len(msg) == 0:
            return None, None
        bmsg = f"<b>⚠️ CPU:</b> {cpu_percent()}% | <b>🥰 FREE:</b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)}"
        bmsg += f"\n<b>⚠️ RAM:</b> {virtual_memory().percent}% | <b>🙋‍♂️ UPTIME:</b> {get_readable_time(time() - botStartTime)}"
        dlspeed_bytes = 0
        upspeed_bytes = 0
